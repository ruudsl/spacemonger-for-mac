...
| Automatic updates | Asks once on first launch, then checks on startup (toggle + "Check Now" in Settings). Uses **Sparkle** for real download/install/relaunch when the package is added ([setup](Distribution/Sparkle.md)); otherwise falls back to a GitHub-Releases check with a **Download** button |
...
These are built in but need extra setup or a signing identity:

- **Real auto-updates with Sparkle** — wired behind `#if canImport(Sparkle)`;
  add the package + an appcast and it downloads/installs/relaunches. Otherwise
  the app falls back to a GitHub-Releases check. See
  [`Distribution/Sparkle.md`](Distribution/Sparkle.md).
- **XPC privileged helper** — app side is integrated (`PrivilegedHelperManager`,
  Settings → install); activates once you add and sign the helper target from
